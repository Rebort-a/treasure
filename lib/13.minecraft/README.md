...
| block.dart | 方块系统：26 种 `BlockType`，覆盖基础地形、矿石、植物、液体、人造方块与红石元件。每种方块具有颜色、透明度和碰撞属性 |
...
| chunk_manager.dart | 区块管理：以玩家为中心加载/卸载区块。首屏同步加载，移动后的新区块按距离排队加载，已生成区块保留在内存缓存 |
| world_generator.dart | 确定性世界生成器：连续分形噪声地形、7 种可达生物群系、海洋/冰面、实体地下石层、洞穴、矿石、树木与仙人掌。结构由坐标哈希决定，可跨区块稳定生成 |
| raycast.dart | 基于统一体素坐标规则的 DDA 射线检测，用于方块选择、破坏和放置 |
| redstone_manager.dart | 自洽的简化红石网络：拉杆/火把供电、红石粉逐格衰减、红石灯响应；网络变化后清空并重新传播，避免残留信号 |
...
| scene_render.dart | 核心渲染协调器：视锥裁剪、背面剔除、近平面裁剪、面合并、投影、光照、雾效与调试覆盖层 |
...
4. 通过多层分形噪声计算连续地形高度
5. 根据温度、湿度、高度和海拔选择生物群系
6. 生成地表、泥土、石层、基岩、洞穴和替换式矿石
7. 低于海平面的区域填充水，寒冷区域形成冰面
8. 通过坐标哈希生成树木和仙人掌，结果不受区块生成顺序影响
```
...
| **生物(Mob)** | 动物/怪物/NPC | 无 |
| **合成/背包** | 完整合成系统 + 物品栏 | 无 |
| **红石电路** | 完整逻辑电路系统 | 简化网络：拉杆/火把、粉尘衰减、红石灯 |
| **多人游戏** | 客户端-服务器架构 | 单人离线 |
| **存档系统** | 区域文件(.mca)持久化 | 内存运行，退出即丢失 |
| **水/岩浆** | 流体传播、物理交互 | 仅渲染为半透明方块，可穿透 |
| **天气/昼夜** | 动态天气 + 昼夜循环 | 固定天空背景 |
| **音效** | 位置音效系统 | 无 |
| **地形复杂度** | 洞穴/矿脉/结构(村庄/要塞) | 连续地形、海洋、洞穴、矿石、树木和仙人掌 |
| **玩家交互** | 挖掘/放置/攻击/使用 | 移动、跳跃、挖掘、拾取、快捷栏、放置和拉杆交互 |
...
| loadChunkHorizontalCount | 2 | 水平加载半径，形成 5×5 区块区域 |
| loadChunkVerticalCount | 1 | 垂直加载半径 |
| renderDistance | 30 | 水平渲染距离 |
| renderVerticalDistance | 14 | 垂直渲染距离 |
| gravity | 20 | 重力加速度 |
| moveSpeed | 3.0 | 移动速度 |
...
坐标哈希生成格点值，再通过平滑插值和多八度叠加形成连续分形噪声。
相同世界种子与坐标始终得到相同结果，且区块生成顺序不会影响世界：

```dart
double _fbm2(double x, double z, int salt) {
  // 叠加多层不同频率和振幅的平滑 value noise
  // 低频决定大陆起伏，高频补充局部细节
}
```
